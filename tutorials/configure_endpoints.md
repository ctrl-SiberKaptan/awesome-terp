...
The `grpc.Server` is a concrete gRPC server, which spawns and serves all gRPC query requests and a broadcast transaction request. This server can be configured inside `~/.terp/config/app.toml`:
...
:::tip
`~/.terp` is the directory where the node's configuration and databases are stored. By default.
:::
...
All routes are configured under the following fields in `~/.terp/config/app.toml`:
...
* some additional API configuration options are defined in `~/.terp/config/app.toml`, along with comments, please refer to that file directly.
...
Enabling the `/swagger` endpoint is configurable inside `~/.terp/config/app.toml` via the `api.swagger` field, which is set to true by default.
